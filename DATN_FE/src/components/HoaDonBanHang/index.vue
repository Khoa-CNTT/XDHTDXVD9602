<template>
    <div class="card">
            ...
            <table class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <td class="align-middle">
                            <input v-on:change="loadData()" type="date" class="form-control" v-model="tk.end" :max="maxEndDate" />
                        </td>
                    ...
                    <tr>
                        ...
                        <th class="align-middle text-center">Nhân Viên Thanh Toán</th>
                        <th class="align-middle text-center">Tiền Trước Giảm</th>
                        <th class="align-middle text-center">Phần Trăm Giảm</th>
                        <th class="align-middle text-center">Tổng Tiền Đã Giảm</th>
                        <th class="align-middle text-center">Ghi Chú</th>
                        <th class="align-middle text-center">Chi Tiết Hóa Đơn</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(v, k) in hoa_don_ban_hangs.data" :key="k">
                        ...
                        <td class="align-middle text-center">
                            <button v-on:click="ghi_chu = v.ghi_chu" type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#ghiChuModal">
                                <i class="fa-solid fa-comment"></i>
                            </button>
                        </td>
                        <td class="align-middle text-center">
                            <button v-on:click="chiTietHoaDon(v)" type="button" class="btn btn-primary me-1" data-bs-toggle="modal" data-bs-target="#chiTietModal">
        ...
        <div class="card-footer text-end text-nowrap">
            <button @click="loadData(currentPage - 1)" :disabled="currentPage <= 1" class="btn btn-secondary btn-sm me-2">Previous</button>
            <button class="btn btn-secondary btn-sm me-2" v-for="page in pageNumbers" :key="page" @click="page !== '...' && loadData(page)" :class="{ active: page === currentPage }">
                {{ page }}
            </button>
            <button @click="loadData(currentPage + 1)" :disabled="currentPage >= totalPages" class="btn btn-secondary btn-sm me-2">Next</button>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        chiTietHoaDon(payload) {
            baseRequest.post("admin/hoa-don/chi-tiet-hoa-don", payload).then((res) => {
